...
- **Rotation**: every muscle group's "days since last trained" is ranked on Home, so you always know what's next.
- **Progress**: Strength Index, and a Records list of every lift's best set grouped by muscle group (collapsed by default). Tap a lift for a Weight-style detail screen: latest top set, change over a selectable range, a scrubbable estimated-1RM chart with your best marked, and a per-month summary.
